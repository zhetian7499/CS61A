...
def balanced(m):
    ...
    if is_planet(m):
        ...
        li,ju=length(left(m))*total_weight(leftend),length(right(m))*total_weight(rightend)
        return li==ju and balanced(leftend) and balanced(rightend)
...
def replace_loki_at_leaf(t, lokis_replacement):
    ...
    if is_leaf(t) and label(t)=='loki':
        ...
        br=[replace_loki_at_leaf(b,lokis_replacement) for b in branches(t)]
...
def sub_interval(x, y):
    ...
    and any value in y."""
    neg=interval(-upper_bound(y),-lower_bound(y))
    return add_interval(x,neg)
