...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    n,i,sad,score,point= num_rolls,0,False,0,0
    while i<n:
        point=dice()
        if point==1:
            sad=True
        i,score=i+1,point+score
    return 1 if sad else score
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    averge_dice=make_averaged(roll_dice,trials_count)
    i,max=1,0
    num=1
    while(i<=10):
        score=averge_dice(i,dice)
        if score>max:
            num=i
            max=score
        i+=1
