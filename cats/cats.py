...
def accuracy(typed, reference):
    ...
    correct = 0
    if total == 0 and len(reference)!=0:
        return 0.0
    if total == 0 and len(reference)==0:
        return 100.0  
...
def fastest_words(match):
    ...
    for i in player_indices:
        ...
        for j in word_indices:
            if min(player_indices,key=lambda x: time(match,x,j))==i:
                player.append(word_at(match,j))
        wordlist.append(player)
    return wordlist    
    # END PROBLEM 10
